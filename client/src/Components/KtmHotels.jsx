...
const KtmHotels = () => {
  ...
  return (
    <>
      ...
      <Carousel responsive={responsive} slidesToSlide={1} arrows={true}>
        {cityKtm.map((hotel) => {
          return (
            
           <div key={hotel._id} className="card px-2  rounded-lg h-96 flex flex-col justify-evenly">
             <img src={`${API}/${hotel.photos[0]}`} alt="" className="rounded-lg object-cover"  />
             ...
             <p className="">
              <Link to={`/hotel/${hotel._id}`} className="w-full btn bg-blue-500 text-white hover:bg-blue-600">Check Availablity</Link>
             </p>
           </div>
          );
        })}
